...
function database(databaseName) {
  ...
  database.createLearningJourney = async (
    learningJourneyName,
    staffId,
    jobRoleId
  ) => {
  ...
  database.createLearningJourneySkill = async (learningJourneyId, skills) => {
    let str = "";
    for (let skill of skills) {
      str += `(${learningJourneyId},${skill.Skill_ID}),`;
    }

    const insertLearningJourneySkill = `INSERT INTO learning_journey_skill (Learning_Journey_ID, Skill_ID) VALUES ${str.slice(
      0,
      -1
    )}`;
    try {
  ...
  database.getSkillByMultipleId = async (skills) => {
    ...
    const getSkill = `SELECT * FROM skill WHERE Is_Active=TRUE AND ${str.slice(
      0,
      -4
    )}`;
  ...
  database.updateRoleDetails = async (
    roleID,
    roleName,
    roleDescription,
    jobDepartment
