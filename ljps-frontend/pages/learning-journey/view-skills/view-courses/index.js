...
export default function ViewCourses() {
  ...
  useEffect(() => {
    if (skillID && roleID) {
      const url = `${process.env.BACKEND}/api/skills/${skillID}`
      ...
      const roleUrl = `${process.env.BACKEND}/api/roles/${roleID}`
      axiosFn(roleUrl)
        .then(function (response) {
          if (response.data.success) {
      ...
      const skillUrl = `${process.env.BACKEND}/api/courses/skill/${skillID}`;
      axiosFn(skillUrl)
        .then(function (response) {
          if (response.data.success) {
