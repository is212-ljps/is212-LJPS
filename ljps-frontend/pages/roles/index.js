...
export default function JobRolesPage() {
  ...
  const onRolesUpdate = useCallback(() => {
    if (active) {
      ...
      axios.get(`${process.env.NEXT_PUBLIC_BACKEND}/api/roles/?active=false`).then(res => {
        setRoles(res.data.data)
      })
    }
  }, [active])
      ...
      <table className="table table-borderless">
        <thead>
          <tr className=" rounded">
            <th scope="col" style={{ ...styles.tableHeaderBg, ...styles.tableBorderRadiusLeft, width: "7%" }}>Role ID</th>
            <th scope="col" style={{ ...styles.tableHeaderBg, width: "15%" }}>Role Name</th>
            <th scope="col" style={{ ...styles.tableHeaderBg, width: "48%" }}>Role Description</th>
            <th scope="col" style={{ ...styles.tableHeaderBg, width: "15%" }}>Role Department</th>
            <th scope="col" style={{ ...styles.tableHeaderBg, ...styles.tableBorderRadiusRight, width: "15%" }}></th>
          </tr>
        </thead>
        <tbody>
